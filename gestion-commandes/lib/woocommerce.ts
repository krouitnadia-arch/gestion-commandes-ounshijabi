...
function extraireVariantes(li: WooLineItem) {
  ...
  for (const o of autres) {
    if (!couleur) couleur = o.valeur;
    else options.push(`${o.cle} : ${o.valeur}`);
  }
...
export function wooOrderToLocal(order: WooOrder) {
  const lignes = order.line_items || [];

  // Le total ne compte que les articles : la livraison choisie sur le site est ignoree
  const totalArticles = lignes.reduce((somme, li) => somme + (parseFloat(li.total || "0") || 0), 0);
  ...
  return {
    ...
    produits: lignes.map((li) => {
      ...
      return {
        ...
        taille: v.taille,
        options: v.options,
        produitId: li.product_id || null,
        variationId: li.variation_id || null,
      };
    }),
    total: Math.round(totalArticles * 100) / 100,
