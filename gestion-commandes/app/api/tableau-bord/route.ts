...
export async function GET() {
  ...
  // Les charges du magasin viennent en deduction du revenu du magasin
  const listeCharges = await prisma.charge.findMany({
    where: { date: { gte: mois } },
  ...
  for (const c of listeCharges) {
    const montant = c.montant || 0;
    chargesMois += montant;
    if (new Date(c.date).getTime() >= jour.getTime()) chargesJour += montant;
  }
  ...
  return NextResponse.json({
    ...
    magasin: {
      ...
      chargesMois: Math.round(chargesMois),
      chargesJour: Math.round(chargesJour),
      nbCharges: listeCharges.length,
      netMois: Math.round(caVentesMois - chargesMois),
    },
    stock: {
