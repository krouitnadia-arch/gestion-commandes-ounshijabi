...
export async function GET() {
  ...
  try {
    // Les commandes deja livrees ne sont plus interrogees : un colis livre
    // ne bouge plus, et cela laisse la place aux colis encore en route.
    const commandes = await prisma.order.findMany({
      where: {
        senditCode: { not: null },
        statut: { notIn: ["LIVREE", "RETOURNEE", "ANNULEE", "SUPPRIMEE_SENDIT"] },
      },
      orderBy: { updatedAt: "desc" },
      take: 60,
    });

    // On affiche aussi le statut des colis livres, sans les reinterroger
    const dejaLivrees = await prisma.order.findMany({
      where: { senditCode: { not: null }, statut: "LIVREE" },
      select: { senditCode: true },
      take: 200,
    });

    for (const c of dejaLivrees) {
      if (c.senditCode) statuts[c.senditCode] = "DELIVERED";
    }
    ...
    for (const c of commandes) {
      ...
      try {
        ...
        // -----------------------------------------------------------
        // Colis livre : la commande passe au statut Livree
        // -----------------------------------------------------------
        if (cote.status === "DELIVERED") {
          const frais = cote.fee > 0 ? cote.fee : Math.round(c.fraisLivraison || 0);
          ...
          await prisma.order.update({
            ...
            data: {
              ...
              senditEtat: { ...cote, fee: frais } as any,
            },
          });

          livrees.push(c.numero);
          continue;
        }

        // -----------------------------------------------------------
        // Toujours pilote par Sendit : les frais et la zone
        // -----------------------------------------------------------
