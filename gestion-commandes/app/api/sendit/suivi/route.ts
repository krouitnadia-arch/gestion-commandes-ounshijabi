...
export async function GET() {
  ...
  for (const c of commandes) {
    ...
    // ---------------------------------------------------------------
    // Le colis n'existe plus chez Sendit : la commande passe au statut
    // "Supprimee sur Sendit" et les pieces reviennent au stock
    // ---------------------------------------------------------------
    if (r.supprime) {
    ...
    function arbitrer(champ: "name" | "phone" | "address" | "comment", cible: string) {
      ...
      const chezSendit = String((cote as any)[champ] || "");
      const chezApp = String((app as any)[champ] || "");
  ...
  return NextResponse.json({ ok: true, statuts, supprimees, descendues, montees, conflits });
}
