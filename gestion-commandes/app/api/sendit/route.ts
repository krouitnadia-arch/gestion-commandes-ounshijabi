...
// Sendit attend un numero local a 10 chiffres : 0XXXXXXXXX
...
export async function POST(req: NextRequest) {
  ...
  const telephone = normaliserTelephone(order.clientTelephone);
  const ramassage = Number(process.env.SENDIT_PICKUP_DISTRICT_ID);

  if (telephone.length !== 10) {
    return NextResponse.json(
      { error: `Numero de telephone invalide : ${order.clientTelephone}` },
      { status: 400 }
    );
  }
