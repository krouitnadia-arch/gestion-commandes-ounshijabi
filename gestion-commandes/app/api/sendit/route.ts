...
function construireColis(order: any, districtId: number) {
  ...
  // Le montant a encaisser par le livreur = prix des articles + frais de
  // livraison. Sans les frais, Sendit encaisserait un montant incomplet.
  const aEncaisser = (order.total || 0) + (order.fraisLivraison || 0);

  return {
    pickup_district_id: Number(process.env.SENDIT_PICKUP_DISTRICT_ID),
    district_id: districtId,
    name: (order.clientNom || "Client").trim(),
    amount: Math.max(0, Math.round(aEncaisser)),
    address: adresse || "-",
...
export async function POST(req: NextRequest) {
  ...
  // "forcer" permet de recreer un colis supprime chez Sendit
  if (order.senditCode && !body.forcer) {
