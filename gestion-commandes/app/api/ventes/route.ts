...
export async function POST(req: NextRequest) {
  ...
  for (const e of entrees) {
    ...
    // Si l'article est lie au site, on y renvoie la nouvelle quantite
    if (produit.wooId) {
      try {
...
// Annulation d'une vente : le stock est remis
export async function DELETE(req: NextRequest) {
  ...
  for (const l of lignes) {
    ...
    const nouvelle = (produit.quantite || 0) + (Number(l.quantite) || 0);
    await prisma.product.update({
      where: { id: produit.id },
      data: { quantite: nouvelle },
    });
