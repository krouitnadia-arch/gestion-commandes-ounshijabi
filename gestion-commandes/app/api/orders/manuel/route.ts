...
export async function POST(req: NextRequest) {
  ...
  // ---------------------------------------------------------------
  // Les commandes Instagram sont servies depuis le stock MAGASIN.
  // ---------------------------------------------------------------
  ...
  for (const e of entrees) {
    ...
    const disponible = p.quantiteMagasin || 0;

    if (disponible <= 0) {
      return NextResponse.json(
        { error: `${p.nom} : aucune piece au magasin, commande impossible` },
        { status: 400 }
      );
    }
    ...
    if (disponible < quantite) {
      return NextResponse.json(
        { error: `${p.nom} : seulement ${disponible} piece(s) au magasin` },
        { status: 400 }
      );
    }
