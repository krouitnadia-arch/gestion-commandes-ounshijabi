...
export async function POST() {
  ...
  try {
    ...
    for (const wo of wooOrders) {
      ...
      // ---------------------------------------------------------------
      // Tant que la commande est encore "Nouvelle", le site reste la
      // reference et on la rafraichit. Des qu'elle change de statut,
      // c'est-a-dire des que l'equipe la prend en main, l'application
      // devient la reference : le site n'ecrase plus ni le total, ni les
      // articles, ni les coordonnees de la cliente.
      // ---------------------------------------------------------------
      if (existing.statut === "NOUVELLE") {
        ...
        const supplement = manuelles.reduce((somme, l) => somme + (Number(l?.total) || 0), 0);

        data.clientNom = local.clientNom;
        data.clientTelephone = local.clientTelephone;
        data.clientAdresse = local.clientAdresse;
        data.clientVille = local.clientVille;
        data.produits = [...local.produits, ...manuelles] as any;
        data.total = local.total + supplement;
      }

      // L'annulation sur le site remonte toujours, quel que soit le statut
