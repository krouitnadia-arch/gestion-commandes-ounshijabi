...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (body.statut) {
    ...
    const ancien = existante.statut;
    const nouveau = body.statut;
    data.statut = nouveau;

    // ---------------------------------------------------------------
    // On passe EN annulee : le colis est supprime et les pieces
    // reviennent au stock.
    // ---------------------------------------------------------------
    if (nouveau === "ANNULEE" && ancien !== "ANNULEE") {
    ...
    // ---------------------------------------------------------------
    // On SORT d'annulee (annulation faite par erreur) : les pieces
    // repartent du stock, sinon le stock resterait gonfle.
    // ---------------------------------------------------------------
    if (ancien === "ANNULEE" && nouveau !== "ANNULEE") {
