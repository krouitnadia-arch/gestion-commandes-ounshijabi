...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  // Une commande Instagram a ete servie depuis le magasin par l'application :
  // c'est la seule dont les pieces reviennent automatiquement.
  const gereParApp = existante.wooId < 0;
  ...
  if (body.statut) {
    ...
    if (devientRendu && !etaitRendu) {
      ...
      if (gereParApp) {
        ...
        // Commande du site : l'application ne touche a rien, elle vous
        // rappelle simplement de remettre les pieces sur ounshijabi.com
        data.stockARemettre = true;
        avertissements.push("Pensez a remettre les articles en stock sur le site");
      }
    }
    ...
    if (etaitRendu && !devientRendu) {
      if (gereParApp) {
        ...
        if (lignes.length > 0) {
          ...
          if (d.retirees > 0) {
            avertissements.push(`${d.retirees} piece(s) retiree(s) du stock magasin`);
          }
        }
      } else {
        data.stockARemettre = false;
      }
    }
  }

  // Le bouton "Remis au stock" de la rubrique Commandes
  if (body.stockARemettre !== undefined) {
    data.stockARemettre = Boolean(body.stockARemettre);
  }
  ...
  if (body.senditDistrictId !== undefined) {
    const zone = Number(body.senditDistrictId);
    data.senditDistrictId = !Number.isNaN(zone) && zone > 0 ? zone : null;
  }
