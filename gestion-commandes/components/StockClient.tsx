...
export default function StockClient({ lectureSeule = false }: { lectureSeule?: boolean }) {
  ...
  async function valider(p: Produit, champ: string) {
    ...
    else if (data?.avertissement) setMessage(data.avertissement);
  ...
  async function fusionnerArticle(g: Groupe) {
    ...
    if (!res.ok) {
      setMessage(data?.error || "Erreur");
    } else if (data?.avertissements && data.avertissements.length > 0) {
      setMessage(data.avertissements.join(" | "));
    } else {
      setMessage(
        L(`${data?.fusionnees || 0} variante(s) fusionn\u00E9e(s)`, `${data?.fusionnees || 0}`)
      );
    }
  ...
  return (
    <div>
      ...
      {lectureSeule && (
        ...
        >
          {L(
            "Stock en consultation : les quantit\u00E9s ne peuvent pas \u00EAtre modifi\u00E9es depuis ce compte.",
            "\u0644\u0644\u0627\u0637\u0644\u0627\u0639 \u0641\u0642\u0637"
          )}
        </p>
      )}
      ...
      {origine === "autres" && !lectureSeule && (
        ...
        >
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <input
              type="number"
              placeholder={L("Quantit\u00E9 par variante", "\u0627\u0644\u0643\u0645\u064A\u0629")}
              value={quantite}
                      ...
                      tri.map((p, i) => {
                        ...
                        return (
                          <tr key={p.id} style={{ background: "#fafafa" }}>
                            ...
                            <td style={{ whiteSpace: "nowrap" }}>
                              {!lectureSeule && (p.quantiteMagasin || 0) > 0 && (
                                <button
                                  type="button"
                                  style={boutonFusion}
