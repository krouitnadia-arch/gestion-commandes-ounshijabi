...
export default function StockClient() {
  ...
  async function fusionnerArticle(g: Groupe) {
    const ok = window.confirm(
      L(
        `Remettre tout le stock magasin de "${g.nom}" sur le site ?`,
        `\u062F\u0645\u062C "${g.nom}" \u061F`
      )
    );
    if (!ok) return;
          ...
          <table>
            ...
            <tbody>
              {groupes.map((g) => {
                ...
                const faible = g.items.some(
                  (x) => (x.quantite || 0) + (x.quantiteMagasin || 0) <= x.seuilAlerte
                );
                ...
                return (
                  <Fragment key={g.cle}>
                    <tr style={{ background: faible ? "#fff7ed" : undefined }}>
                      ...
                      <td style={{ fontWeight: 700, color: magasin > 0 ? "#0369a1" : undefined }}>
                        {magasin}
                      </td>
                    ...
                    {ouvert && (
                      <tr style={{ background: "#eef2ff" }}>
                        ...
                        <td style={{ fontSize: 11, fontWeight: 700 }}>{motSite}</td>
                        <td style={{ fontSize: 11, fontWeight: 700 }}>{motMagasin}</td>
                        ...
                        return (
                          <tr key={p.id} style={{ background: "#f8fafc" }}>
                            ...
                            <td>
                              <input
                                type="number"
                                defaultValue={p.quantiteMagasin}
                                onBlur={(e) => modifier(p, "quantiteMagasin", e.target.value)}
                                className="notes-input"
                                style={{ width: 70, fontWeight: 700, color: "#0369a1" }}
                              />
                            </td>
