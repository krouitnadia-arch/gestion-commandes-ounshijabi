...
// Les ventes en boutique se servent uniquement dans le stock magasin
function stockMagasin(p: Produit) {
  return p.quantiteMagasin || 0;
}
...
export default function MagasinClient({ lectureSeule = false }: { lectureSeule?: boolean }) {
  ...
  function ajouterAuPanier(p: Produit) {
    const dispo = stockMagasin(p);

    if (dispo <= 0) {
      setMessage(
        L(
          `${p.nom} : aucune pi\u00E8ce au magasin, vente impossible`,
          `${p.nom} : \u0646\u0641\u0627\u062F \u0627\u0644\u0645\u062E\u0632\u0648\u0646`
        )
      );
      return;
    ...
    if (existe) {
      if (existe.quantite + 1 > dispo) {
        setMessage(
          L(`${p.nom} : seulement ${dispo} pi\u00E8ce(s) au magasin`, `${p.nom} : ${dispo}`)
        );
        return;
      }
  ...
  async function annuler(v: Vente) {
    const ok = window.confirm(
      L(
        "Annuler cette vente ? Les pieces reviendront au stock magasin.",
        "\u0625\u0644\u063A\u0627\u0621 \u061F"
      )
    );
    if (!ok) return;
        ...
        <div>
          ...
          <form onSubmit={ajouterCharge} className="stock-form">
            ...
            <button type="submit" className="btn-primary" disabled={envoiCharge}>
              {envoiCharge ? "..." : L("Ajouter la charge", "\u0625\u0636\u0627\u0641\u0629")}
            </button>
          </form>
          ...
          <form onSubmit={enregistrer}>
            ...
            {lignesInvalides.length > 0 && (
              <p style={{ color: "#b91c1c", fontWeight: 700, fontSize: 13, margin: "0 0 10px" }}>
                {L(
                  "Quantit\u00E9 sup\u00E9rieure au stock du magasin : la vente ne peut pas \u00EAtre enregistr\u00E9e.",
