"use client";

import Link from "next/link";
import { usePathname, useRouter } from "next/navigation";
import { useLang } from "./LangProvider";
import LanguageSwitcher from "./LanguageSwitcher";
import {
  IconeAccueil,
  IconeCommandes,
  IconeExpedition,
  IconeRetours,
  IconeProduction,
  IconeMagasin,
  IconeStock,
  IconeUtilisateurs,
} from "./Icones";

type Role = "ADMIN" | "EMPLOYE" | "CONFIRMATION" | "EXPEDITION" | "STOCK";

const LIENS: {
  href: string;
  fr: string;
  ar: string;
  roles: Role[];
  icone: (p: { taille?: number }) => JSX.Element;
}[] = [
  {
    href: "/",
    fr: "Accueil",
    ar: "\u0627\u0644\u0631\u0626\u064A\u0633\u064A\u0629",
    roles: ["ADMIN", "EMPLOYE", "CONFIRMATION", "EXPEDITION", "STOCK"],
    icone: IconeAccueil,
  },
  {
    href: "/commandes",
    fr: "Commandes",
    ar: "\u0627\u0644\u0637\u0644\u0628\u0627\u062A",
    roles: ["ADMIN", "EMPLOYE", "CONFIRMATION"],
    icone: IconeCommandes,
  },
  {
    href: "/expedition",
    fr: "Exp\u00E9dition",
    ar: "\u0627\u0644\u0634\u062D\u0646",
    roles: ["ADMIN", "EMPLOYE", "EXPEDITION"],
    icone: IconeExpedition,
  },
  {
    href: "/retours",
    fr: "Retours",
    ar: "\u0627\u0644\u0645\u0631\u062A\u062C\u0639\u0627\u062A",
    roles: ["ADMIN", "EMPLOYE", "EXPEDITION", "STOCK"],
    icone: IconeRetours,
  },
  {
    href: "/production",
    fr: "Production",
    ar: "\u0627\u0644\u0625\u0646\u062A\u0627\u062C",
    roles: ["ADMIN"],
    icone: IconeProduction,
  },
  {
    href: "/magasin",
    fr: "Magasin",
    ar: "\u0627\u0644\u0645\u062A\u062C\u0631",
    roles: ["ADMIN", "EMPLOYE", "STOCK"],
    icone: IconeMagasin,
  },
  {
    href: "/stock",
    fr: "Stock",
    ar: "\u0627\u0644\u0645\u062E\u0632\u0648\u0646",
    roles: ["ADMIN", "EMPLOYE", "STOCK"],
    icone: IconeStock,
  },
  {
    href: "/utilisateurs",
    fr: "Utilisateurs",
    ar: "\u0627\u0644\u0645\u0633\u062A\u062E\u062F\u0645\u0648\u0646",
    roles: ["ADMIN"],
    icone: IconeUtilisateurs,
  },
];

export default function Sidebar({ role, name }: { role: string; name: string }) {
  const { t, lang } = useLang();
  const pathname = usePathname();
  const router = useRouter();

  async function logout() {
    await fetch("/api/auth/logout", { method: "POST" });
    router.push("/login");
    router.refresh();
  }

  function actif(href: string) {
    if (href === "/") return pathname === "/";
    return pathname.startsWith(href);
  }

  return (
    <aside className="sidebar">
      <div className="sidebar-logo">
        <img src="/logo.svg" alt="Ouns Hijabi" />
      </div>

      <nav className="sidebar-nav">
        {LIENS.filter((l) => l.roles.includes(role as Role)).map((l) => {
          const Icone = l.icone;
          return (
            <Link key={l.href} href={l.href} className={actif(l.href) ? "active" : ""}>
              <Icone taille={19} />
              <span>{lang === "ar" ? l.ar : l.fr}</span>
            </Link>
          );
        })}
      </nav>

      <div className="sidebar-footer">
        <LanguageSwitcher />
        <div className="sidebar-user">{name}</div>
        <button className="btn-link" onClick={logout} type="button">
          {t("nav_logout")}
        </button>
      </div>
    </aside>
  );
}
