...
function stockTotal(a: ArticleStock) {
  return (a.quantite || 0) + (a.quantiteMagasin || 0);
}
...
export default function OrdersClient() {
  ...
  // Le lien WhatsApp est construit a l'avance : l'ouverture doit etre
  // immediate au clic, sinon les telephones bloquent la fenetre.
  // Le statut ne change pas tout seul : vous le passez a "Appelee" a la main
  // dans le menu deroulant, une fois le message reellement envoye.
  ...
  async function ajouterArticleCommande(order: Order, a: ArticleStock) {
    if (stockTotal(a) <= 0) {
      setMessage(
        L(
          `${a.nom} : stock \u00E9puis\u00E9, ajout impossible`,
          `${a.nom} : \u0646\u0641\u0627\u062F \u0627\u0644\u0645\u062E\u0632\u0648\u0646`
        )
      );
      return;
  ...
  function ajouterAuPanier(a: ArticleStock) {
    const dispo = stockTotal(a);

    if (dispo <= 0) {
      setMessage(
        L(
          `${a.nom} : stock \u00E9puis\u00E9, vente impossible`,
          `${a.nom} : \u0646\u0641\u0627\u062F \u0627\u0644\u0645\u062E\u0632\u0648\u0646`
        )
      );
      return;
    ...
    if (existe) {
      if (existe.quantite + 1 > dispo) {
        setMessage(L(`${a.nom} : seulement ${dispo} pi\u00E8ce(s) en stock`, `${a.nom} : ${dispo}`));
        return;
      }
        ...
        <form onSubmit={ajouterCommande} style={{ marginBottom: 18 }}>
          ...
          {lignesInvalides.length > 0 && (
            <p style={{ color: "#b91c1c", fontWeight: 700, fontSize: 13, margin: "0 0 10px" }}>
              {L(
                "Quantit\u00E9 sup\u00E9rieure au stock disponible : la commande ne peut pas \u00EAtre enregistr\u00E9e.",
