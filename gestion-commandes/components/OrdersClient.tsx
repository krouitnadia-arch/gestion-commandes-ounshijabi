...
export default function OrdersClient() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (!silencieux || data.nouvelles > 0) {
          setMessage(L(`Synchronise : ${data.nouvelles} nouvelle(s)`, `${data.nouvelles}`));
        }
        await loadOrders();
      } else {
  ...
  async function handleStatusChange(order: Order, statut: OrderStatus) {
    if (statut === "ANNULEE" && order.senditCodePresent) {
      // securite : jamais atteint, la propriete n'existe pas
    }
    ...
    <div>
      ...
      {onglet === "web" && (
        <p className="sync-msg">
          ...
          {derniereSync
            ? ` - ${derniereSync.toLocaleTimeString(ar ? "ar" : "fr-FR")}`
            : ""}
        </p>
      )}
                    ...
                    <tr key={l.productId}>
                      ...
                      <td>
                        <button
                          ...
                          onClick={() => setPanier(panier.filter((x) => x.productId !== l.productId))}
                        >
                          {t("delete")}
                        </button>
                      </td>
