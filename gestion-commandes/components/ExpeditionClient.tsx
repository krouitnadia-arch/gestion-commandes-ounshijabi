...
// Toutes les zones qui correspondent a la ville de la cliente.
// Quand il y en a plusieurs (Casablanca et ses secteurs), on n'en choisit
// aucune : c'est a vous de trancher, plutot qu'un secteur au hasard.
...
export default function ExpeditionClient() {
  ...
  // Initialisation de la zone : d'abord celle deja enregistree dans la
  // commande, sinon la ville si elle ne correspond qu'a une seule zone.
  useEffect(() => {
    ...
    setZoneTexte((actuel) => {
      ...
      for (const o of orders) {
        ...
        if (o.senditDistrictId) {
          ...
          if (enregistree) {
            suivant[o.id] = enregistree.name;
            nouveaux[o.id] = String(enregistree.id);
            continue;
          }
        }
        ...
        if (correspondances.length === 1) {
          suivant[o.id] = correspondances[0].name;
          nouveaux[o.id] = String(correspondances[0].id);
        } else {
          suivant[o.id] = "";
        }
      }
  ...
  async function choisirZone(orderId: string, valeur: string) {
    ...
    // Le choix est enregistre tout de suite : il ne sera plus recalcule
    if (d) {
      await fetch(`/api/orders/${orderId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ senditDistrictId: d.id }),
      });
    }
  }
                      ...
                      <td>
                        ...
                        {zone ? (
                          ...
                          <div style={{ fontSize: 10, color: "#b45309", marginTop: 3 }}>
                            {L(
                              `${correspondances.length} secteurs - choisissez`,
