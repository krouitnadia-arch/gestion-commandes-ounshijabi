...
export default function ExpeditionClient() {
  ...
  async function chargerSuivi() {
    ...
    try {
      ...
      if (res.ok && data.annulees && data.annulees.length > 0) {
        setMessage(
          L(
            `Colis disparus chez Sendit, commandes annulees : ${data.annulees.join(", ")}`,
            data.annulees.join(", ")
          )
        );
        await load();
  ...
  async function envoyerSendit(order: Order, forcer: boolean) {
    ...
    try {
      ...
      if (res.ok) {
        setMessage(L(`Colis cree : ${data.code}`, `${data.code}`));
        await load();
        chargerSuivi();
      } else {
  ...
  async function majColis(order: Order) {
    ...
    try {
      ...
      if (res.ok) {
        setMessage(L(`Colis mis a jour : ${data.code}`, `${data.code}`));
        await load();
        chargerSuivi();
      } else {
        setMessage(data.error || "Erreur");
  ...
  const statutsPresents = Array.from(
    new Set(
      orders
        .map((o) => (o.senditCode ? statuts[o.senditCode] : null))
                        ...
                        <div>
                          ...
                          {!introuvable && (
                            <button
                              ...
                              disabled={occupe}
                            >
                              {occupe ? "..." : L("Mettre a jour le colis", "\u062A\u062D\u062F\u064A\u062B")}
                            </button>
                          )}
                          <button
                            type="button"
                            ...
                            disabled={occupe}
                          >
                            {occupe ? "..." : L("Renvoyer a Sendit", "\u0625\u0639\u0627\u062F\u0629 \u0627\u0644\u0625\u0631\u0633\u0627\u0644")}
                          </button>
                        </div>
                      ) : (
                        <button
